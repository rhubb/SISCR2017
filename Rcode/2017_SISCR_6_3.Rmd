...
- We will also be using the _eha_ and  _survival_ packages for traditional survival regression
...
sof.nomiss <- na.omit(sof[,c("id","totbmd","visit","bmi","age_base")])    
    
sof.jags <- list(totbmd = sof.nomiss$totbmd, visit = sof.nomiss$visit, bmi = sof.nomiss$bmi, age_base = sof.nomiss$age_base, id = rep(seq(1,length(unique(sof.nomiss$id))),times=table(sof.nomiss$id)), n = length(sof.nomiss$id), m = length(unique(sof.nomiss$id)))  
    ```
  ...
  b. Write a JAGS model specifying the likelihood and priors.
    ...
    model{
      ## likelihood
         for (i in 1:n){
            ...
            mu[i] <- beta0 + beta1*visit[i] + beta2*bmi[i] + beta3*age_base[i] + b0[id[i]]
         }   
    ...
    ```{r, eval = TRUE}
# define jags model
mod <- jags.model(file=textConnection(sof.model), data=sof.jags, inits=list(beta0 = 0, beta1 = 0, beta2 = 0, beta3 = 0, b0 = rep(0,length(unique(sof.nomiss$id))), sigmasq = 0.01, sigmabsq = 0.01), n.chains=2, n.adapt=1000)

# update the model    
update(mod, n.iter = 1000)
    
# specify parameters to be monitored
params <- c("beta0","beta1","beta2","beta3","sigmasq","sigmabsq")

# run jags and save posterior samples
samps <- coda.samples(mod, params, n.iter=1000)

# summarize posterior samples
summary(samps)
summary(window(samps, start=1000))
plot(samps)    
    ```

  d. Use diagnostic plots and statistics to evaluate model convergence. If it appears that your chains have not converged, try increasing the number of adaptation samples or number of iterations sampled in (c).
...
3. How do your results obtained using _JAGS_ compare to those from _INLA_? Using, JAGS try out several prior different prior distributions. Are results sensitive to your choice of prior?
...
```
  f. Meta-analysis accounting for between trial heterogeneity via random-effects and allowing for effects modification by cholesterol reduction
    ```{r, eval = TRUE}
